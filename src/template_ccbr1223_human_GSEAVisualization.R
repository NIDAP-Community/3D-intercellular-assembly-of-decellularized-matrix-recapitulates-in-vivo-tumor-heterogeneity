# GSEA Visualization - Sugarloaf V1 [CCBR] (20896e36-771a-419f-a987-f90726837351): v415
ccbr1223_human_GSEAVisualization <- function(ccbr1223_human_DEG, ccbr1223_human_GSEAFiltered, ccbr1223_human_BatchCorrectedCounts, ccbr1223_human_metadata, msigdb_v6_2_with_orthologs) {
...
## PARAMETERS
    ...
    genescore_df <- ccbr1223_human_DEG
    gex_df <- ccbr1223_human_BatchCorrectedCounts
    metadata_df <- ccbr1223_human_metadata
    gsea_df <- ccbr1223_human_GSEAFiltered
...
if (grepl("LE", which_plot)) {
    ...
    samples_to_include = setdiff(c("ECM_HT29_1","ECM_HT29_2","ECM_HT29_3","ECM_HT29_4","ECM_HT29_5","ECM_HT29_6","CA_HT29_1","CA_HT29_2","CA_HT29_3","CA_HT29_4","CA_HT29_5","CA_HT29_6"), geneid)
...
for ( i in 1:length(gsea_list) ) {
    ...
    if (which_plot == "ES+RNK+LE") {
        ...
        metadata = prep.metadata(meta=metadata_df, gsea=gsea, samples=colnames(gex_le), Sample="Sample", Group="Treatment_CellLine", transformation=gex_transformation, dropREF=drop_ref, reference=reference_phenotype, own_palette=c(), label_palette='Accent', filterSamples=filter_samples)
...
png(filename="ccbr1223_human_GSEAVisualization.png", width=(pageWidth+5)*300, height=(pageHeight+5)*300, units="px", pointsize=4, bg="white", res=300, type="cairo") 
print(preview)
...
print("template_ccbr1223_human_GSEAVisualization.R #########################################################################")
...
# Processing input variable: var_ccbr1223_human_DEG
var_ccbr1223_human_DEG<-readRDS(paste0(rds_output,"/var_ccbr1223_human_DEG.rds"))

if (!('Seurat' %in% class(var_ccbr1223_human_DEG))) { if (!(class(var_ccbr1223_human_DEG) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_ccbr1223_human_DEG <- as.data.frame(var_ccbr1223_human_DEG)}}
#############################


# Processing input variable: var_ccbr1223_human_GSEAFiltered
var_ccbr1223_human_GSEAFiltered<-readRDS(paste0(rds_output,"/var_ccbr1223_human_GSEAFiltered.rds"))

if (!('Seurat' %in% class(var_ccbr1223_human_GSEAFiltered))) { if (!(class(var_ccbr1223_human_GSEAFiltered) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_ccbr1223_human_GSEAFiltered <- as.data.frame(var_ccbr1223_human_GSEAFiltered)}}
#############################
...
# Processing input variable: var_msigdb_v6_2_with_orthologs
var_msigdb_v6_2_with_orthologs<-readRDS(paste0(rds_output,"/var_msigdb_v6_2_with_orthologs.rds"))

if (!('Seurat' %in% class(var_msigdb_v6_2_with_orthologs))) { if (!(class(var_msigdb_v6_2_with_orthologs) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_msigdb_v6_2_with_orthologs <- as.data.frame(var_msigdb_v6_2_with_orthologs)}}
#############################

# Saving Output Dataset
invisible(graphics.off())
var_ccbr1223_human_GSEAVisualization<-ccbr1223_human_GSEAVisualization(var_ccbr1223_human_DEG,var_ccbr1223_human_GSEAFiltered,var_ccbr1223_human_BatchCorrectedCounts,var_ccbr1223_human_metadata,var_msigdb_v6_2_with_orthologs)
saveRDS(var_ccbr1223_human_GSEAVisualization, paste0(rds_output,"/var_ccbr1223_human_GSEAVisualization.rds"))
